...
void AARectangle::RotateAroundPoint(const float Angle, const Vec2D& CenterPoint)
{
...
bool AARectangle::ContainsPoint(const Vec2D& point)
{
	bool withinX = point.GetX() >= GetTopLeftPoint().GetX() && point.GetX() <= GetBottomRightPoint().GetX();
	bool withnY = point.GetY() >= GetTopLeftPoint().GetY() && point.GetY() <= GetBottomRightPoint().GetY();
