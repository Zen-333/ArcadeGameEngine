...
void Ball::MakeFlushWithEdge(const BoundaryEdge& edge, Vec2D& pointOnEdge, bool limitTiEdge)
{
	...
	pointOnEdge = edge.edge.ClosestPoint(mBBox.GetCenterPoint(), limitTiEdge);
}
...
void Ball::Bounce(const BoundaryEdge& edge)
{
	...
	mVelocity = mVelocity.Reflet(edge.normal);
