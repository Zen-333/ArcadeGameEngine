...
void Paddle::Update(uint32_t dt, Ball& ball)
{
	...
	if(mDirection != 0)
	{
		...
		if(IsGreaterThanOrEual(mBoundary.GetTopLeftPoint().GetX(), aaRect.GetTopLeftPoint().GetX()))
		{
			MoveTo(Vec2D(mBoundary.GetTopLeftPoint().GetX(), aaRect.GetTopLeftPoint().GetY()));
		}else if(IsGreaterThanOrEual(aaRect.GetBottomRightPoint().GetX(), mBoundary.GetBottomRightPoint().GetX()))
		{
...
bool Paddle::Bounce(Ball& ball)
{
	...
	if(HasCollided(ball.GetBoundingRect(), edge))
	{
		...
		ball.SetVelocity(ball.GetVelocity().Reflet(edge.normal));
		return true;
